...
if ($role == "user") {
    ...
    $deliveredorders = mysqli_query($conn, "SELECT * FROM `orders` WHERE mail ='$mail' AND status = 'delivered' ");
    $takenorders = mysqli_query($conn, "SELECT * FROM `orders` WHERE mail ='$mail' AND status = 'order taken - food on table in 5 min' ");
    // $pending = mysqli_query($conn, "SELECT * FROM `orders` WHERE mail ='$mail' AND status = 'pending' ");
$pendingorders = mysqli_query(
    $conn,
    "SELECT * FROM `orders` WHERE mail ='$mail' AND TRIM(LOWER(status)) = 'pending'"
);
